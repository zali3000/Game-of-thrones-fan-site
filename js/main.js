...
function initDetailCards(){
 ...
 const openCard=card=>{
   ...
   const sections=makeSections(detail,page,finalTitle,clone);
   content.innerHTML=`<div class="detail-layout"><div class="detail-media-column">${image}<div class="detail-media-caption"><span>${escapeHTML(section)}</span><b>ARCHIVE ENTRY</b></div></div><div class="detail-info"><span class="detail-kicker">${escapeHTML(section)}</span><h2>${escapeHTML(finalTitle)}</h2><div class="detail-divider"></div><div class="detail-copy">${sections}</div></div></div>`;
