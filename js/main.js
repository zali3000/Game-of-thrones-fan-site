...
const makeHouseModal=(detail,title,clone,era)=>{
  const motto=clone.querySelector('.motto')?.textContent?.trim() || '';
  const region=clone.querySelector('.region')?.textContent?.trim() || '';
  const seat=clone.querySelector('.seat')?.textContent?.trim() || '';
  const lore=clone.querySelector('.lore')?.textContent?.trim() || detail?.description || '';
  const houseName=title.replace(/^House\s+/i,'').trim();
  const members=Object.entries(CHARACTER_PROFILES)
    .filter(([key,p])=>key.startsWith(`${era}/characters/`) && normalize(String(p?.house||'')).includes(normalize(houseName)))
    .map(([,p])=>p?.known||p?.name||'')
    .filter(Boolean);
  const uniqueMembers=[...new Set(members)].slice(0,12);
  return `<div class="detail-layout house-detail-layout"><div class="detail-media-column">${clone.querySelector('.house-photo img') ? `<div class="detail-image-wrap"><img src="${escapeHTML(clone.querySelector('.house-photo img').getAttribute('src'))}" alt="${escapeHTML(title)}" loading="lazy" decoding="async"></div>`:''}<div class="detail-media-caption"><span>${escapeHTML(region)}</span><b>HOUSE ARCHIVE</b></div></div><div class="detail-info"><span class="detail-kicker">${escapeHTML(era==='hotd'?'House of the Dragon':'Game of Thrones')} · HOUSE</span><h2 id="detailModalTitle">${escapeHTML(title)}</h2><p class="character-role">${escapeHTML(motto)}</p><div class="detail-divider"></div><div class="character-meta"><div><b>REGION</b><span>${escapeHTML(region)}</span></div><div><b>SEAT</b><span>${escapeHTML(seat.replace(/^Seat:\s*/i,''))}</span></div><div><b>MOTTO</b><span>${escapeHTML(motto.replace(/^"|"$/g,''))}</span></div></div><div class="character-detail-sections"><section class="detail-section"><h3>House History</h3><p>${escapeHTML(lore)}</p></section>${uniqueMembers.length?`<section class="detail-section"><h3>Notable Members in This Archive</h3><div class="house-members-list">${uniqueMembers.map(m=>`<span class="house-member-chip">${escapeHTML(m)}</span>`).join('')}</div></section>`:''}<section class="detail-section"><h3>House Identity</h3><p>${escapeHTML(`${title} is represented in this archive through its seat, motto, regional identity, and recorded connections to characters and other Westerosi entries.`)}</p></section></div></div></div>`;
};
